...
export const KIND_REGISTRY: Record<Kind, KindInfo> = {
  ...
  'step.research': {
    ...
    category: 'step',
    description:
      'researcher actor 단계 (v0.3.0) — 3 reference games × 3 actionable lessons synthesis. video evidence 가 있을 때 evidence_refs 로 step.research.video 연결.',
    emitter: 'researcher',
  ...
  'step.research.video': {
    ...
    category: 'step',
    description:
      'researcher actor 단계 (v0.3.0) — per-clip video evidence (mechanic / timing / palette extraction via gemini-sdk Files API + YouTube URL Part). metadata.cache_key 로 replay dedup.',
    emitter: 'researcher',
  ...
  'step.judge': {
    ...
    ref: 'v0.1 §3.4 + agents/verifier.md',
  },
  'user.intervene': {
    kind: 'user.intervene',
    category: 'user',
