...
export interface QaResult {
  ...
  /**
   * v0.5 PR-Controls — Phaser.Game.isBooted true (Stage 1 of the boot probe).
   * `phaser_scene_running=false` + `phaser_booted=true` indicates the first
   * scene is waiting on user input that the spec.controls block does not
   * list. Set whenever runPlaywrightSmoke runs (skipped → undefined).
   */
