...
Distilled wiki pages are written in **English**, regardless of the source document's language. Translate Korean / Japanese / Chinese sources into English when distilling; keep at most one short verbatim quote per page (in source language with an English inline gloss when non-English) so attribution stays honest. Reasons:
...
- Crumb's root `CLAUDE.md` mandates English for the codebase + commits + PRs (the wiki is a codebase artifact, not chat).
