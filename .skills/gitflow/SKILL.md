---
name: gitflow
description: Branch strategy + PR rules. feature → develop → main merge flow (or feature → main when single-branch), Pre-PR Quality Gate (CI guardrails + docs-sync loop), Post-PR CI ratchet (gh pr checks --watch mandatory), Korean PRs, assignee settings. Triggers on "branch", "git", "pr", "merge", "커밋", "풀리퀘스트".
---

> Adapted from GEODE for Crumb. When applying: Crumb currently uses a single `main` branch (no `develop`); collapse the two-step merge flow to feature → main when needed. Replace `core/` with `src/`, `uv run` with `npm run` / `npx tsx`, `pytest` with `vitest`. The CI ratchet philosophy and Pre-PR Quality Gate apply unchanged.

# Crumb Git & PR Workflow
