...
## Language policy — wiki content is English-only

All wiki pages (`wiki/concepts/`, `wiki/synthesis/`, `wiki/findings/`, `wiki/references/`, including frontmatter, body, headings, citations, code-comment glosses) are written in **English**. Korean is reserved for design discussion in conversation and ephemeral chat — it does not enter the wiki. Reasons:

- The Bagelcode hiring panel + future open-source contributors are the wiki's audience; English is the lowest-friction reading surface for both.
- A wiki rebuild / lint / cross-link pass operates on English token boundaries; mixed-language pages create false positives in `wiki-lint` and miss hits in `wiki-query`.
- Crumb's root `CLAUDE.md` already mandates English for the codebase + commits + PRs (the wiki is a codebase artifact, not chat).

If a draft was originally drafted in Korean (e.g. inside a conversation), translate it as the final step before saving — never commit the Korean form. Citation blocks stay in their source language: English papers stay English; the rare Korean source quotes the Korean inline plus an English gloss. When in doubt, write English.
