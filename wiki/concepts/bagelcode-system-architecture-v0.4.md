---
title: Crumb 시스템 구조 v0.4 — AC predicates + judge-input isolation + numerical self-bias discount
category: concepts
...
summary: >-
  Crumb v0.4 incremental layer over v0.1. v0.1 의 9 architecture invariants 는 그대로 유지되고
  ...
  9 invariants 추가 — 8: fallback audit trail, 9: judge isolation. v0.4 는 v0.1 의 schema /
...
# Crumb 시스템 구조 v0.4 — AC predicates + judge-input isolation + numerical discount

> **읽는 순서**: [[bagelcode-system-architecture-v0.1]] 가 baseline. 이 페이지는 v0.1 → v0.4 의 차이만 다룸.
> 다이어그램은 [[bagelcode-system-diagrams-v0.4]] 에 분리.

---

## 0. 한 줄 정체성

> v0.4 는 v0.1 의 multi-host × 3-tuple actor binding 위에 **anti-deception (R1→R7 + numerical Rule 4)**, **AC predicate 결정론 ground truth**, **judge-input hard isolation** 3 축을 얹은 incremental release. v0.1 의 9 invariants 위에 2 invariants 추가 (총 11).

---

## 1. v0.1 → v0.4 변경 요약 (한 표)
...
v0.1 의 metadata 14 필드는 그대로. 다만 다음 필드의 *enforcement* 가 v0.4 에서 격상:
...
| **R7** | verdict=PASS but ac_results contains FAIL | **cap D1≤2, downgrade→PARTIAL** | `verify_pass_with_ac_failure` | **v0.4 신규** (ArtifactsBench 2024) |

R4 의 시간 변천:
- v0.1.0-v0.3.0: violation tag 만 push, action 없음
- v0.3.1: binary `if PASS → PARTIAL`
- **v0.4**: numerical D1+D3+D5 ×0.85 (D2/D6 deterministic 면역, D4 reducer-auto 면역). `combineAggregate` 가 LLM 절반만 깎이도록 자동 처리. wiki: [[bagelcode-same-provider-discount-2026-05-03]].
...
[[bagelcode-system-diagrams-v0.4]] 참조 — Mermaid 6 종:
...
- [[bagelcode-system-diagrams-v0.4]] — Mermaid 6 종
