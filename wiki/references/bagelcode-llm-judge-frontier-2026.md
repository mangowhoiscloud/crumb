...
summary: >-
  ...
  G-Eval CoT+form-filling, position bias / self-bias 연구, multi-judge consensus 97-98% F1. Crumb v3
...
# LLM-as-Judge Frontier — 6 사료 (Crumb v3 scoring 학술 backbone)

> [[bagelcode-system-architecture-v3]] §7 의 3 layer scoring (reducer + qa_check effect + verifier CourtEval) 와 §7.3 의 anti-deception 5 룰의 1차 학술 근거.
...
**Crumb 적용**:
- v3 §7.1 Layer 3 — verifier sandwich 안 inline 4 sub-step (`step.judge.{grader,critic,defender,regrader}`)
...
**Crumb 적용**:
- v3 §"Adaptive stopping" — `progress_ledger.score_history` variance < 1.0 over 2 rounds → next=done
...
**Crumb 적용**:
- v3 §3.6 `kind=judge.score.scores` 의 form-filling 구조 (D1-D6 + source / lookup / evidence)
...
**Crumb 적용**:
- v3 §"bagelcode-cross-3way" preset 의 builder=Codex / verifier=Gemini 매핑이 정확히 cure
...
- v3 §3.6 metadata.cross_provider=true flag 가 평가자에게 self-bias 회피 인지
...
**Crumb 적용**:
- v3 §"latency 1 spawn" — verifier 1 spawn 추가 (CourtEval 4 sub-step inline) ≈ +500-800ms
- 동일 production 패턴, 동일 latency profile

---

## 종합 — Crumb v3 scoring 3 layer 의 frontier 정합

| Crumb v3 결정 | 매핑 사료 | 강도 |
|---|---|---|
...
- [[bagelcode-system-architecture-v3]] — 본 사료의 Crumb 통합
- [[bagelcode-rubric-scoring]] — D1-D6 1차 spec (v3 §7 가 3 layer 로 확장)
...
- [[bagelcode-final-design-2026]] §3.D — CourtEval 1차 도입 (v3 가 3 layer 로 확장)
