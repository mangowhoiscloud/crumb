...
>
> **v0.1 변경 요지** (`wiki/concepts/bagelcode-system-architecture-v0.1.md`):
...
생략할 kind: `ack`, `audit`, `tool.call`, `tool.result`, `agent.thought_summary`. 살릴 kind: `step.judge` 의 grader/critic/defender/regrader 4 sub-step + `qa.result` (v0.1 신규 deterministic ground truth).
...
| `sandwich_append: <text>` | fact `category='sandwich_append'` 추가 — dispatcher 가 이후 모든 매칭 spawn 의 시스템 프롬프트 에 concatenate (v0.2.0 G4) |
...
### 6. 중단 후 재개 (v0.1 persistence boost)
...
v2 의 "Engineering Lead 안 inline 4 sub-step" → v0.1 의 **builder + verifier 별도 actor** (cross-provider true split).
...
- `wiki/concepts/bagelcode-system-architecture-v0.1.md` — ★ canonical v0.1 시스템 구조 (multi-host + 3-tuple + 5 actor + 3-layer scoring + MCP Provider + persistence)
...
- `wiki/concepts/bagelcode-system-architecture.md` (v2) — §3-§9 (envelope / cache / per-turn flow / control plane / OTel) v0.1 에서도 유효
