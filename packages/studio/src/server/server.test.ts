...
describeServer('studio server', () => {
  it('serves studio HTML at /', async () => {
    ...
    try {
      // React client bundle (or the "not built" fallback when dist/client/
      // is absent in CI without a prior `npm run build`).
      const r = await fetchText(server.url);
      expect(r.status === 200 || r.status === 503).toBe(true);
      expect(r.body).toMatch(/<!doctype html>/i);
      // Both the live bundle and the fallback share the "Crumb" title prefix.
