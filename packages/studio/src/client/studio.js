...
// v0.4.4 adapter-auth-detail helpers — translate raw plan strings + epoch
// expiry into compact human-readable chips. Plan dictionary mirrors the
// canonical values doctor.ts probes from claude keychain / codex JWT /
// gemini OAuth blob (see doctor.ts §"Subscription / plan tier" comment).
const ADAPTER_PLAN_LABEL = {
  ...
  free: 'Free',
  // OpenAI ChatGPT chatgpt_plan_type (from auth.json id_token JWT)
...
function formatAdapterPlan(a) {
  if (!a.plan) return '';
  return ADAPTER_PLAN_LABEL[a.plan] ?? a.plan;
}
function formatAdapterExpiry(iso) {
...
function adapterExpiryClass(iso) {
  const ms = Date.parse(iso) - Date.now();
  if (ms <= 0) return 'expired';
  if (ms < 60 * 60 * 1000) return 'soon'; // < 1h — amber
  return 'ok';
}
...
function renderAdapterList() {
  ...
  root.innerHTML = adapterCache
    .map((a) => {
      ...
      // v0.4.4 adapter-auth-detail: when auth is confirmed, surface plan tier
      // (claude max / codex prolite / gemini advanced / apikey) and remaining
      // login lifetime as small subtle chips next to the pill. The keychain /
      // config-file probe in doctor.ts populates these without spawning a CLI.
      const planChip =
        a.authenticated === true && a.plan
          ? '<span class="adapter-plan" title="' +
            escapeHTML(
              'plan: ' +
                a.plan +
                (a.email ? ' · ' + a.email : '') +
                (a.auth_source ? ' · src: ' + a.auth_source : ''),
            ) +
            '">' +
            escapeHTML(formatAdapterPlan(a)) +
            '</span>'
          : '';
      const expiryChip =
        a.authenticated === true && a.login_expires_at
          ? '<span class="adapter-expiry ' +
            adapterExpiryClass(a.login_expires_at) +
            '" title="' +
            escapeHTML('login expires ' + a.login_expires_at) +
            '">' +
            escapeHTML(formatAdapterExpiry(a.login_expires_at)) +
            '</span>'
          : '';
      return (
        '<div class="' +
