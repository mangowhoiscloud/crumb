...
이 도구는 멀티 에이전트 협업을 단순 채팅이 아니라 **관찰·개입·평가·재현 가능한 실행 프로토콜**로 다룹니다. 모든 메시지·도구 호출·산출물·사용자 개입·deterministic QA 결과·judge 점수가 **35 kind × 11 field × 13 specialist step × 9 actor** transcript 한 줄로 영속.
...
| "여러 AI 에이전트가 **서로 통신**" | 9 actor × 35 transcript kind, actor 간 `kind=handoff.{requested,rollback}`. |
         ...
         visual)             code-review,
              ...
              └──────────────────┴────────────┬─────┘
                            ...
                            emits kind=qa.result (D2/D6 ground truth)
                                              │
                                              ▼
                              transcript.jsonl (35 kind, append-only)
                                              │
                                              ▼
...
- **스키마**: 35 kind × 11 field × 13 specialist step × 9 actor × OTel GenAI alias
...
```
✅ 스키마 v0.1 — 35 kind × 11 field × 13 step × 9 from + D1-D6 source-of-truth scoring
